...
#
# The suite is ~51 s at -n auto and nothing in it is slow on its own - the cost is how often
# it runs. Measured over 30 sessions, Claude ran it whole 475 times, ~16 a session and 26 in
...
`scripts/verify.py --no-tests --site <site>` (~6 s) for geometry. The full suite runs ONCE,
